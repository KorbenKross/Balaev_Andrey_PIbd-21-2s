    ...
    public class ReportServiceBD : IReportService
    {
        ...
        public void SaveProductPrice(ReportConnectingModel model)
        {
            ...
            try
            {
                ...
                var products = context.Ingredients.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].IngredientName;
        ...
        public List<ProductStorageLoadUserViewModel> GetStocksLoad()
        {
            return context.ProductStorages
                            .ToList()
                            .GroupJoin(
                                    context.ProductStorageElements
                                                .Include(r => r.Element)
                                                .ToList(),
                                    stock => stock,
                                    stockComponent => stockComponent.ProductStorage,
                                    (stock, stockCompList) =>
            new ProductStorageLoadUserViewModel
            {
                ProductStorageName = stock.ProductStorageName,
            ...
            try
            {
                ...
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        excelcells.Value2 = elem.ProductStorageName;
        ...
        public List<BuyerOrderModel> GetClientOrders(ReportConnectingModel model)
        {
            return context.CustomerSelections
                            .Include(rec => rec.Buyer)
                            .Include(rec => rec.Ingredient)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new BuyerOrderModel
                            {
        ...
        public void SaveClientOrders(ReportConnectingModel model)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].IngredientName, fontForCells));
                table.AddCell(cell);
