...
namespace VirtualStoreView
{
    public partial class FormGeneral : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                List<CustomerSelectionUserViewModel> list = Task.Run(() => APIClient.GetRequestData<List<CustomerSelectionUserViewModel>>("api/General/GetList")).Result;
                if (list != null)
                {
        ...
        private void buttonOrderReady_Click(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                ...
                Task task = Task.Run(() => APIClient.PostRequestData("api/General/FinishOrder", new CustomerSelectionModel
                {
                    Id = id
                }));
        ...
        private void buttonPayOrder_Click(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                ...
                Task task = Task.Run(() => APIClient.PostRequestData("api/General/PayOrder", new CustomerSelectionModel
                {
                    Id = id
                }));
        ...
        private void прайсЛистToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                ...
                Task task = Task.Run(() => APIClient.PostRequestData("api/Report/SaveProductPrice", new ReportConnectingModel
                {
                    FileName = fileName
                }));
